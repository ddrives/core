...
function Cursor (vault, name, opts) {
  if (!(this instanceof Cursor)) return new Cursor(vault, name, opts)
  ...
  function open (cb) {
    vault.stat(name, function (err, st) {
      ...
      vault._ensureContent(function (err) {
        if (err) return cb(err)
        if (!st.isFile()) return cb(new Error('Not a file'))

        self._content = vault.content
        self._stat = st
